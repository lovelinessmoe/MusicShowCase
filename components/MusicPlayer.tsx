...
export function MusicPlayer({ track }: MusicPlayerProps) {
  ...
  return (
    <div className={`w-full mx-auto ${hasLyrics ? 'max-w-7xl' : 'max-w-2xl'}`}>
      <div className={`flex ${hasLyrics ? 'flex-col lg:flex-row' : 'justify-center'} gap-6 items-stretch`}>
        {/* 播放器主体 */}
        <div className={`${hasLyrics ? 'w-full lg:w-auto lg:flex-shrink-0' : 'w-full max-w-2xl'} mx-auto lg:mx-0`}>
          <div className="w-full p-4 sm:p-6 lg:p-8 bg-white/5 backdrop-blur-lg rounded-2xl shadow-2xl max-w-[280px] sm:max-w-[400px] lg:max-w-[500px] border border-white/10 transition-all duration-300 hover:bg-white/10 hover:border-white/20 mx-auto h-full">
            {/* 专辑封面 */}
            <AlbumCover
              src={track.coverUrl}
        ...
        {/* 歌词显示区域 - 在右侧，与左边等高 */}
        {hasLyrics && (
          <div className="w-full lg:flex-1 p-6 bg-gradient-to-br from-white/5 via-blue-500/5 to-purple-500/5 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/10 relative overflow-hidden">
            {/* 背景装饰 */}
